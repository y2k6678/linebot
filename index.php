...
function showtime($time)
{
	...
	{
		$h[1] = 45;
		$selectbydate = "select * from weatherstation where \"DATETIME\" BETWEEN '2017-09-17 $h[0]:45:01' and '2017-09-17 $h[0]:59:00'";
	}
...
if (!is_null($events['events']))
{
	...
	foreach($events['events'] as $event)
	{
		...
		if ($event['type'] == 'message' && $event['message']['type'] == 'text')
		{
			...
			{
				$messages = ['type' => 'text', 'text' => "อุณหภูมิ C :" . $TEM];
			}

			if (trim($text) == "ความชื้น")
			{
				$messages = ['type' => 'text', 'text' => "ความชื้น: " . $HUM . " %"];
			}

			if (trim($text) == "2")
			{
				$messages = ['type' => 'text', 'text' => "ค่าแสง : " . "\n" . "1500 - 800 = กลางคืน" . "\n" . "800-500 = เช้ามืด" . "\n"];
			}
